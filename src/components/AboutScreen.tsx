...
export function AboutScreen({ isDarkMode }: AboutScreenProps) {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto h-full flex flex-col animate-in fade-in duration-500">
      ...
      <div className="flex-1 flex flex-col justify-center min-h-0 space-y-3 md:space-y-4">
        <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 shrink-0">
          Funcionalidades que ajudam seu aprendizado
        </h2>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 overflow-y-auto pr-1">
          {features.map((feature, index) => (
            <div 
              key={index}
              className="bg-[#1A1A24] dark:bg-slate-800 p-4 md:p-5 rounded-xl shadow-lg border border-transparent dark:border-slate-700 hover:transform hover:-translate-y-1 transition-all duration-300 flex flex-col justify-center"
            >
              <h3 className="text-base font-bold text-white mb-1.5">
                {feature.title}
              </h3>
              <p className="text-blue-100 dark:text-slate-300 text-sm leading-snug">
                {feature.description}
              </p>
            </div>
          ))}
      ...
      <div className="shrink-0 flex flex-col items-center text-center space-y-4 pt-4 md:pt-6 mt-2 border-t border-slate-200 dark:border-slate-800/50">
        <div className="flex items-center gap-6 md:gap-8 w-full max-w-3xl justify-center">
            ...
            <a 
              href="#" 
              className="group relative p-3 md:p-4 bg-slate-100 dark:bg-slate-800 rounded-full transition-all duration-300 hover:scale-110 hover:shadow-xl hover:bg-white dark:hover:bg-slate-700 shrink-0"
            >
              ...
              <Github className="w-10 h-10 md:w-12 md:h-12 text-slate-800 dark:text-white" />
            </a>
